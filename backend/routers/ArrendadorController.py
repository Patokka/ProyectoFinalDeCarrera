...
def crear_arrendador(dto: ArrendadorDto, db: Session = Depends(get_db)):
...
def actualizar_arrendador(arrendador_id: int, dto: ArrendadorDtoModificacion, db: Session = Depends(get_db)):
...
def eliminar_arrendador(arrendador_id: int, db: Session = Depends(get_db)):
