...
export default function SideBar({ children }: { children: ReactNode }) {
  ...
  if (!mounted || !user) return <>{children}</>; // No muestra el sidebar si no hay sesión
    ...
    <ProtectedRoute>
      <div className="relative left-4 bg-gray-50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6 lg:gap-8">
          {sidebarVisible && (
            <aside className="lg:col-span-1 space-y-4 p-4 bg-slate-200 border-r border-gray-400 transition-all duration-300 sticky top-16 overflow-y-auto h-[calc(100vh-4rem)]">
              <Calendar/>
              <PaymentSummary/>
              <QuintalesSummary/>
