...
export default function ModificarArrendadorPage() {
    ...
    const guardarArrendador = async () => {
        ...
        try {
            ...
            if (msg.includes("CUIT - CUIL ya está registrado")) {
                ...
                setErrors(prev => ({ ...prev, nombre: "El nombre ya está registrado para otro arrendador" }));
                toast.error(msg);
            } else{
                toast.error(msg);
            }
    ...
    useEffect(() => {
        ...
        const loadArrendador = async () => {
            ...
            try {
                ...
                setArrendador(arr);
                setFormData({
                    nombre_o_razon_social: arr.nombre_o_razon_social || '',
                    cuil: arr.cuil || '',
    ...
    useEffect(() => {
        if (arrendador && arrendador.localidad) {
            if (provincias.length > 0) {
                const provinciaEncontrada = provincias.find((p) => Number(p.value) === arrendador.localidad.provincia.id);
                if (provinciaEncontrada) {
                    setProvinciaActual(provinciaEncontrada);
                }
            }
            if (localidades.length > 0) {
                const localidadEncontrada = localidades.find((l) => Number(l.value) === arrendador.localidad.id);
    ...
    <ProtectedRoute allowedRoles={["ADMINISTRADOR", "OPERADOR"]}>
        <div className="min-h-screen bg-gray-50 p-6">
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mt-6">
                    <Link href={`/arrendadores/${idArrendador}`} passHref>
