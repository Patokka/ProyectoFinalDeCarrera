...
export default function ArrendatariosPage() {
  ...
  const filteredData = useMemo(() => {
    return arrendatarios.filter(item => {
      const matchesNombre = item.razon_social
        .toLowerCase()
...
const handleDelete = (id: number) => {
  ...
  const confirmToastId = toast.info(
    "¿Está seguro que desea eliminar este arrendatario?",
    {
      action: {
        label: "Confirmar",
        onClick: () => {
          ...
          toast.promise(
            ...
            {
              loading: "Eliminando arrendatario...",
              success: "Arrendador eliminado con éxito",
              error: (err) => err.message || "Error al eliminar el arrendatario",
            }
          );
        },
      },
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
            {loading ? (
              ...
              <div className="text-center py-12 text-gray-500">
                No se encontraron arrendatarios que coincidan con los filtros.
              </div>
            ) : (
                      ...
                      <tr key={arrendatario.id} className="hover:bg-gray-50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                          <div className="flex space-x-2">
                            ...
                            <button onClick={() => handleDelete(arrendatario.id)} className={`p-1 rounded transition-colors ${canEditEliminate ? "text-red-600 hover:text-red-900 hover:bg-red-50 cursor-pointer": "text-gray-400 cursor-not-allowed"}`} title="Eliminar"
                                    disabled={!canEditEliminate}>
